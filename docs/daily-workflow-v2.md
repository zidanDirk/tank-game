...
服务器已核验：Ubuntu、2 CPU/约 2 GiB RAM、Node 22.22.1、Claude Code 2.1.276。Issue #9 的历史浏览器日志证实 Chromium 缓存路径不存在；安装后又发现缺少 Linux 动态库。已安装 Chromium 与其系统依赖，并在改写 HOME 的干净环境下成功启动。新版完整服务器回归、真实研究发布与自动合并仍待验证。
...
目标为 1–3 个修改文件、100–250 行有效改动；默认硬上限 350 行。只对 changeFiles 计数，回归检查不算修改文件。验收 1–3 条，单一可交付行为。每个切片自身必须可测试，不把所有测试留到最后一个切片。验证路径、字段类型、依赖无环、估算与范围；缺失完整路径不能视为零文件或自动通过。最终 diff 再次检查真实范围。
